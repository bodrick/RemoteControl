...
public class ScreenCaster : IScreenCaster
{
    ...
    {
        _ = Task.Run(() => BeginScreenCastingImplAsync(screenCastRequest));
    }

    private async Task BeginScreenCastingImplAsync(ScreenCastRequest screenCastRequest)
    {
        try
        {
            ...
            await viewer.SendViewerConnectedAsync();

            await viewer.SendScreenDataAsync(
                viewer.Capturer.SelectedScreen,
                viewer.Capturer.GetDisplayNames(),
                screenBounds.Width,
                screenBounds.Height);

            await viewer.SendCursorChangeAsync(_cursorIconWatcher.GetCurrentCursor());
            ...
            {
                await viewer.SendScreenCaptureAsync(new ScreenCaptureDto()
                {
            ...
            await viewer.SendDesktopStreamAsync(GetDesktopStreamAsync(viewer), screenCastRequest.StreamId);
        }
        catch (Exception ex)
        {
    ...
    private async IAsyncEnumerable<byte[]> GetDesktopStreamAsync(IViewer viewer)
    {
        _metricsCts?.Cancel();
        _metricsCts?.Dispose();
        ...
        try
        {
            _ = Task.Run(async () => await LogMetricsAsync(viewer, _metricsCts.Token));
    ...
    private async Task LogMetricsAsync(IViewer viewer, CancellationToken cancellationToken)
    {
