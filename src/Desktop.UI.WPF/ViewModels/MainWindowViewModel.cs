...
public class MainWindowViewModel : BrandedViewModelBase, IMainWindowViewModel
{
        ...
        : base(brandingProvider, dispatcher, logger)
    {
        ...
        RemoveViewersCommand = new AsyncRelayCommand<IList<object>>(RemoveViewersAsync, CanRemoveViewers);
    }
    ...
    public async Task InitAsync()
    {
        ...
        try
        {
            var result = await _hubConnection.ConnectAsync(_dispatcher.ApplicationExitingToken, TimeSpan.FromSeconds(10));
    ...
    private async Task RemoveViewersAsync(IList<object>? viewers)
    {
        if (viewers?.Any() != true)
        {
            return;
    ...
    private async void ScreenCastRequested(object? sender, ScreenCastRequest screenCastRequest)
    {
        await _dispatcher.InvokeWpfAsync(async () =>
        {
            ...
            {
                await _hubConnection.SendConnectionRequestDeniedAsync(screenCastRequest.ViewerID);
            }
        });
    }
