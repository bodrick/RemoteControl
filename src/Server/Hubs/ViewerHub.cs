...
public class ViewerHub : Hub
{
    ...
    public async Task ChangeWindowsSessionAsync(int sessionID)
    {
        if (SessionInfo?.Mode == RemoteControlMode.Unattended)
        {
            await _hubEvents.ChangeWindowsSessionAsync(SessionInfo, Context.ConnectionId, sessionID);
        }
    }

    public async IAsyncEnumerable<byte[]> GetDesktopStreamAsync()
    {
        var result = await _streamCache.WaitForStreamSessionAsync(SessionInfo.StreamId, TimeSpan.FromSeconds(30));
    ...
    public Task InvokeCtrlAltDelAsync()
    {
        return _hubEvents.InvokeCtrlAltDelAsync(SessionInfo, Context.ConnectionId);
    }
    ...
    public Task SendDtoToClientAsync(byte[] dtoWrapper)
    {
        if (string.IsNullOrWhiteSpace(SessionInfo.DesktopConnectionId))
        {
    ...
    public async Task SendScreenCastRequestToDeviceAsync(string sessionId, string accessKey, string requesterName)
    {
