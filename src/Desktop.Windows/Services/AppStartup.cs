...
internal class AppStartup : IAppStartup
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var wpfStarted = await _dispatcher.StartWpfThreadAsync().ConfigureAwait(false);
    ...
    {
        if (!await _desktopHub.ConnectAsync(_dispatcher.ApplicationExitingToken, TimeSpan.FromSeconds(30)))
        {
