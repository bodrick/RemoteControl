...
public class FileTransferServiceLinux : IFileTransferService
{
    ...
    public async Task ReceiveFileAsync(byte[] buffer, string fileName, string messageId, bool endOfFile, bool startOfFile)
    {
        ...
        finally
        {
            ...
            if (endOfFile)
            {
                await Task.Run(ShowTransferCompleteAsync);
            }
        }
    }

    public async Task UploadFileAsync(FileUpload fileUpload, IViewer viewer, CancellationToken cancelToken, Action<double> progressUpdateCallback)
    {
        try
        {
            await viewer.SendFileAsync(fileUpload, cancelToken, progressUpdateCallback);
        }
        catch (Exception ex)
        {
    ...
    private async Task ShowTransferCompleteAsync()
    {
        ...
        if (!_messageBoxPending)
        {
            ...
            await MessageBox.ShowAsync($"File tranfer complete.  Files saved to directory:\n\n{GetBaseDirectory()}",
                "Tranfer Complete",
                MessageBoxType.OK);
