...
public class ChatUiServiceLinux : IChatUiService
{
    ...
    public async Task ReceiveChatAsync(ChatMessage chatMessage)
    {
        await _dispatcher.InvokeAsync(async () =>
        {
            if (chatMessage.Disconnected)
            {
                await MessageBox.ShowAsync("The partner has disconnected.", "Partner Disconnected", MessageBoxType.OK);
                Environment.Exit(0);
                return;
            }
